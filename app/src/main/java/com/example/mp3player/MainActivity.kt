...
class MainActivity : AppCompatActivity() {
    ...
    override fun onRequestPermissionsResult(
        ...
        if(requestCode == REQUEST_CODE){
            if(grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(applicationContext,"권한승인을 해야만 앱을 사용할 수 있습니다",Toast.LENGTH_SHORT).show()
                finish()
            }
        }
    }
    private fun startProcess() {
        ...
        if(cursor!!.count <= 0){
            Toast.makeText(applicationContext,"메모리에 음악파일이 없습니다",Toast.LENGTH_SHORT).show()
            finish()
        }
        while(cursor.moveToNext()){
            ...
            val musicData = MusicData(id, title, artist, albumId, duration, like = null)
            musicDataList.add(musicData)
        }

        // Adapter와 recyclerView
